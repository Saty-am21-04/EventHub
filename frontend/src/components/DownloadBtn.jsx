...
const DownloadBtn = ({ onClick }) => {
  return (
    <div className="download-container relative z-10 scale-75 origin-right">
      <label className="label">
        <input type="checkbox" class="input" onChange={(e) => {
            if(e.target.checked) {
                // Wait for animation to finish (approx 3.5s) before triggering actual download
                setTimeout(() => {
                    onClick();
                    // Reset checkbox after download (optional)
